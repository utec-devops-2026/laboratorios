...
**Configurar secretos en GitHub** (`Settings → Secrets and variables → Actions`):

| Secret                   | Valor                                         |
|--------------------------|-----------------------------------------------|
| `AWS_ACCESS_KEY_ID`      | Access key con permisos ECR + ECS + CloudWatch|
| `AWS_SECRET_ACCESS_KEY`  | Secret key correspondiente                    |
| `GRAFANA_ADMIN_PASSWORD` | Password de admin para Grafana                |
