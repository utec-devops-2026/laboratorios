...
- Configurar entorno reproducible (conda + cookiecutter) pa proyecto MLOps.
- Cubrir flujo end-to-end: raw data → processing → feature engineering → training/experimentación → dockerization → publish modelo → API/UI → monitoring.
- Trackear experimentos y modelos con MLflow.
- Empacar modelo en Docker pa deploy consistente.
- Servir modelo vía API + app Streamlit.
- Monitorear modelo en producción (drift, métricas).
